...
def _argparse():
    parser = argparse.ArgumentParser(add_help=False,
                                     description='解析mysql 5.7/8.0的ibd文件 https://github.com/ddcw/ibd2sql')
    parser.add_argument('--help', '-h', action='store_true', dest="HELP", default=False, help='show help')
    parser.add_argument('--version', '-v', '-V', action='store_true', dest="VERSION", default=False,
                        help='show version')
    ...
    parser.add_argument('--delete', action='store_true', dest="DELETED", default=False,
                        help='print data only for flag of deleted')
    parser.add_argument('--complete-insert', action='store_true', dest="COMPLETE_INSERT", default=False,
                        help='use complete insert statements for sql')
    parser.add_argument('--force', '-f', action='store_true', dest="FORCE", default=False,
                        help='force pasrser file when Error Page')
    parser.add_argument('--set', action='store_true', dest="SET", default=False,
                        help='set/enum to fill in actual data instead of strings')
    parser.add_argument('--multi-value', action='store_true', dest="MULTI_VALUE", default=False,
                        help='single sql if data belong to one page')
    parser.add_argument('--replace', action='store_true', dest="REPLACE", default=False,
                        help='"REPLACE INTO" replace to "INSERT INTO" (default)')
    ...
    parser.add_argument('--debug', '-D', action='store_true', dest="DEBUG", default=False,
                        help="will DEBUG (it's too big)")
    parser.add_argument('--debug-file', dest="DEBUG_FILE", help='default sys.stdout if DEBUG')
    parser.add_argument('--page-min', action='store', type=int, dest="PAGE_MIN", default=0,
                        help='if PAGE NO less than it, will break')
    parser.add_argument('--page-max', action='store', type=int, dest="PAGE_MAX", default=4294967296,
                        help='if PAGE NO great than it, will break')
    ...
    parser.add_argument('--page-skip', action='store', type=int, dest="PAGE_SKIP",
                        help='skip some pages when start parse index page')
    ...
    # keyring file
    parser.add_argument('--keyring-file', '-k', action='store', dest="KEYRING_FILE", default='',
...
if __name__ == '__main__':
    ...
    with open(filename, 'rb') as f:
        ...
        if data != b'\x00' * 115:
            ...
            key_info = AES.aes_ecb256_decrypt(master_key,
                                              data[43 + 4:43 + 4 + 32 * 2]) if ddcw.MYSQL5 else AES.aes_ecb256_decrypt(
                master_key, data[43:43 + 32 * 2])
    ...
    if parser.DEBUG_FILE is not None:
        f = open(parser.DEBUG_FILE, 'a', encoding='utf-8')
        ddcw.DEBUG = True
        ddcw.DEBUG_FD = f
